...
; (function ($) {
    ...
    Hotspot.prototype.setupWorkspace = function () {
        ...
        this.element.delegate('span', 'click', function (event) {
            ...
            $.ajax({
                type: "Post",
                url: "/Exam/SubmitHotspotDetails",
                data: { "ExamId": $('#examId').val(), "LoginId": $("#loginId").val(), "Type": $('#type').val(), "HpDetails": hotspot },
                success: function (result) {
                    if (result.success) {
                        GetHotSpots();
                        Notification("Success", "Submitted Successfully", "success");
                    } else {
                        Notification("Erroe", "Some Error Occured", "danger");
                    }
                }
            })
    ...
    Hotspot.prototype.displaySpot = function (hotspot, unsaved) {
        ...
        $(spot).on('contextmenu', function (e) {
            ...
            $.ajax({
                type: "Post",
                url: "/Exam/DeleteOneSpotData",
                data: { "ExamId": $('#examId').val(), "Type": $('#type').val(), "X": hotspot.x, "Y": hotspot.y },
                success: function (result) {
                    if (result.success) {
