...
;(function($) {
	...
	Hotspot.prototype.setupWorkspace = function() {
		...
		$('<span/>', {
			html: '<p>Click this Pane to Store Your Answers</p>'
		}).css({
		...
		this.element.delegate('span', 'click', function(event) {
            ...
            $.ajax({
                type: "Post",
                url: "/TutorDashboard/SubmitHotspotDetailsForSecondPaper",
                data: { "ExamId": $('#examId').val(), "ImageId": $('#imageId').val(), "HpDetails": hotspot },
			...
			//widget.displaySpot(hotspot, false);
		});
	...
	Hotspot.prototype.displaySpot = function(hotspot, unsaved) {
        ...
        //right click on the hotspot
        $(spot).on('contextmenu', function (e) {
            e.preventDefault();
            ...
            $.ajax({
                type: "Post",
                url: "/TutorDashboard/DeleteOneHotspotDetailsForPapers",
                data: { "ExamId": $('#examId').val(), "ImageId": $('#imageId').val(), "X": hotspot.x, "Y": hotspot.y},
                success: function (result) {
                    if (result.success) {
                        $("#theElement-a .HotspotPlugin_Hotspot").remove();
                        GetHotSpots();
