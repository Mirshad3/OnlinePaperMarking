...
function GetImages() {
    $.ajax({
        url: "/TutorDashboard/GetImagesForFirstPaperCorrections?ExamId=" + $('#examId').val(),
        success: function (result) {
            if (result.success) {
                var i = 0;
...
function EditFirstPaperHotSpot() {
    var imageId = $('.active').find('img').attr('id');
    window.open('/TutorDashboard/EditFirstPaperHotspots?ExamId=' + $('#examId').val() + '&ImageId=' + imageId);
}